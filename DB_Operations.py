...
def create_table_custom_fields_table(connection, cursor) -> None:
    ...
    try:
        ...
        Папка_клиента TEXT,
        Дата_договор DATE, 
        Дата_постоянная_бронь DATE, 
        Бронь_проставлена TEXT, 
        Договор_проставлен TEXT,
        Откуда_узнал TEXT, 
        Дата_завершения DATE, 
        Завершение_проставлено TEXT);"""
        cursor.execute(create_query)
        connection.commit()
...
def create_table_finance_table(connection, cursor) -> None:
    ...
    try:
        ...
        Сумма_четвертого_платежа TEXT,
        Дата_четвертого_платежа TIMESTAMP,
        Итоговая_цена_объекта INT);"""
        cursor.execute(create_query)
        connection.commit()
...
def insert_custom_fields(connection, cursor, records_to_insert: list) -> None:
    ...
    try:
        ...
        insert_query = """INSERT INTO custom_fields_table (
                ...
                Партнер_Агент, Проект, Язык, formname, Классификация_сделки, Папка_клиента, Дата_договор, Дата_постоянная_бронь, Бронь_проставлена, Договор_проставлен) \
...
def insert_finance_table(connection, cursor, records_to_insert: list) -> None:
    ...
    try:
        ...
        insert_query = """INSERT INTO finance_table (
                ...
                Сумма_третьего_платежа, Дата_третьего_платежа, Сумма_четвертого_платежа, 
                Дата_четвертого_платежа, Итоговая_цена_объекта) \
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
        cursor.executemany(insert_query, records_to_insert)
        connection.commit()
    except Exception as error:
...
def update_custom_fields(connection, cursor, records_to_insert: list) -> None:
    ...
    try:
        ...
        update_query = """INSERT INTO custom_fields_table (
                ID, Был_в_Новая_заявка, Был_в_Менеджер_назначен, Был_в_Взята_в_работу, Был_в_Попытка_связаться,
                Был_в_Презентация_отправлена, Был_в_Контакт_состоялся, Был_в_Встреча_назначена, Был_в_Встреча_проведена,
                Был_в_Ожидаем_предоплату,
                Был_в_Получена_предоплата, Был_в_Реквизиты_получены, Был_в_Договор_у_юриста,
                Был_в_Договор_клиенту, Был_в_Договор_согласован,  Был_в_Договор_подписан, Был_в_Первый_платеж,
                Был_в_Второй_платеж, Был_в_Третий_платеж, Был_в_Выиграно, Был_в_Проиграно,
                Дата_Новая_заявка, Дата_Менеджер_назначен, Дата_Взята_в_работу, Дата_Попытка_связаться,
                Дата_Презентация_отправлена, Дата_Контакт_состоялся, Дата_Встреча_назначена,
                Дата_Встреча_проведена, Дата_Ожидаем_предоплату, Дата_Получена_предоплата, Дата_Реквизиты_получены,
                Дата_Договор_у_юриста, 
                Дата_Договор_клиенту, Дата_Договор_согласован,
                Дата_Договор_подписан, Дата_Первый_платеж, Дата_Второй_платеж, Дата_Третий_платеж, Дата_Выиграно,
                Дата_Проиграно, Источник_заявки, Взята, Скорость_взятия, Причина_отказа, Отказ_подробно,
                Партнер_Агент, Проект, Язык, formname, Классификация_сделки, Был_в_Подготовка_договора,
                Был_в_Договор_подготовлен, Был_в_Юристом_согласован, Дата_Подготовка_договора, Дата_Договор_подготовлен,
                Дата_Юристом_согласован, Папка_клиента, Дата_договор, Дата_постоянная_бронь, Бронь_проставлена,
                Договор_проставлен, Откуда_узнал, Дата_завершения, Завершение_проставлено) \
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,
                %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,
                %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                ON CONFLICT (id) DO UPDATE SET
                ...
                Был_в_Подготовка_договора = EXCLUDED.Был_в_Подготовка_договора,
                ...
                Папка_клиента = EXCLUDED.Папка_клиента,
                Дата_договор = EXCLUDED.Дата_договор, 
                Дата_постоянная_бронь = EXCLUDED.Дата_постоянная_бронь, 
                Бронь_проставлена = EXCLUDED.Бронь_проставлена, 
                Договор_проставлен = EXCLUDED.Договор_проставлен,
                Откуда_узнал = EXCLUDED.Откуда_узнал, 
                Дата_завершения = EXCLUDED.Дата_завершения, 
                Завершение_проставлено = EXCLUDED.Завершение_проставлено
                """
        cursor.executemany(update_query, records_to_insert)
        connection.commit()
    except Exception as error:
...
def update_finance_table(connection, cursor, records_to_insert: list) -> None:
    ...
    try:
        ...
        update_query = """INSERT INTO finance_table (
                ...
                Итоговая_цена_объекта = EXCLUDED.Итоговая_цена_объекта
                """
        cursor.executemany(update_query, records_to_insert)
        connection.commit()
    except Exception as error:
